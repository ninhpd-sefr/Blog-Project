...
export function actSetCurrentUser(user) {
  return {
...
export function actUpdateUserProfile(formValues, objFile) {
  return async dispatch => {
    try {
      const data = {
      ...
      if (objFile) {
        const responseMedia = await authService.uploadAvatar(objFile)
        const mediaId = responseMedia.data.id
        data.mediaId = mediaId
      }
  
      const responseMe = await authService.updateMe(data)
      // delete media by old id
      const user = mappingCurrentUser(responseMe.data)
